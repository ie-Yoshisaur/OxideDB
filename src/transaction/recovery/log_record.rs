...
// Declare a separate enum to hold the constants
...
impl dyn LogRecord {
    pub fn create_log_record(bytes: Vec<u8>) -> Box<dyn LogRecord> {
        let mut page = Page::new_from_bytes(bytes);

        // Make sure to handle errors; here we unwrap
        let log_record_type = page.get_int(0).unwrap();
