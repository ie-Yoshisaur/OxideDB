...
impl FileManager {
    ...
    pub fn new(db_directory: PathBuf, blocksize: usize) -> Result<FileManager, FileError> {
        let is_new = !db_directory.exists();
        if is_new {
            create_dir_all(&db_directory).map_err(FileError::Io)?;
        }
    ...
    pub fn read(&self, block: &BlockId, page: &mut Page) -> Result<(), FileError> {
    ...
    pub fn write(&self, block: &BlockId, page: &mut Page) -> Result<(), FileError> {
        let mut file = self.get_file(&block.get_file_name())?;
        let file_offset = (block.get_block_number() * self.get_block_size() as i32) as u64;
    ...
    pub fn append(&self, filename: &str) -> Result<BlockId, FileError> {
    ...
    pub fn length(&self, filename: &str) -> Result<usize, FileError> {
    ...
    ///
    /// Returns an error if the file cannot be accessed.
    fn get_file(&self, filename: &str) -> Result<File, FileError> {
        let mut open_files = self.open_files.lock().unwrap();
        if let Some(file) = open_files.get(filename) {
            return Ok(file.try_clone().map_err(FileError::Io)?);
        }
        ...
        open_files.insert(
            filename.to_string(),
            file.try_clone().map_err(FileError::Io)?,
        );
        Ok(file)
    }
}
